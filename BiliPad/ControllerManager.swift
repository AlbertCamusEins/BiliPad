...
enum ControllerAction: String, Sendable {
    case up, down, left, right, confirm, back, playPause, fullscreen, interaction, tripleInteraction, toggleDanmaku, menu, previousTab, nextTab, refresh
}
...
final class ControllerManager: ObservableObject {
    ...
    private func configure(_ controller: GCController) {
        ...
        g.leftThumbstick.valueChangedHandler = { [weak self] _, x, y in Task { @MainActor [weak self] in self?.stickDirection = Self.direction(x, y, 0.55); self?.updateRepeat() } }
