...
BiliPad 是一个面向 iPhone 和手柄操作的实验性原生 B 站客户端。V0.4 已把网页套壳从主流程移除，内容列表、详情、分 P 与播放均使用 SwiftUI/AVPlayer；WebKit 只存在于登录授权页。
...
- 原生热门视频、观看历史、收藏夹、视频详情和分 P 选择
- AVPlayer 解码配合自绘播放画面与控制层，不使用系统 VideoPlayer 控件
- 同机登录：创建 B 站网页授权会话，打开本机哔哩哔哩 App 确认，并自动建立 Cookie 会话
- 登录 Cookie 只保存在 iOS Keychain，不写日志、不进仓库
- 手柄：方向键导航；LB/RB 切换标签；按下左摇杆刷新；播放时 A 播放/暂停、B 先退出全屏再返回、Y 切换全屏、Option 切换自动连播设置、上下调音量、左右快退/快进
- 设置页支持手柄完整导航和持久化按键重映射；选择动作后按下新按键即可绑定，冲突绑定自动交换
- 触屏操作始终可用

## 当前限制

- 推荐首版仍使用热门流，个性化推荐和搜索稍后接入
